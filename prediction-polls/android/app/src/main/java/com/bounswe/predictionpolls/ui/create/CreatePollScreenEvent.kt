...
sealed class CreatePollScreenEvent {
    ...
    data class OnPollTypeChanged(val pollType: CreatePollScreenState.PollType) :
        CreatePollScreenEvent()

    data class OnDiscreteOptionChanged(val option: String, val position: Int) :
        CreatePollScreenEvent()
    ...
    data class OnContinuousInputTypeChanged(val inputType: CreatePollScreenState.ContinuousInputType) :
        CreatePollScreenEvent()
    ...
    data class OnAcceptValueTypeChanged(val type: CreatePollScreenState.AcceptValueType) :
        CreatePollScreenEvent()

    data class OnDistributionVisibilityChanged(val isChecked: Boolean) : CreatePollScreenEvent()
    data class OnCreatePollClicked(
        val onSuccess: () -> Unit,
    ) : CreatePollScreenEvent()
